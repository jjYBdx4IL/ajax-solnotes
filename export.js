/**
 * Scans for notes ending with the line pattern ",PUBLISH:CATEGORY:<cat-name>[,<tag1>][...,<tagN>]", and writes
...
function exportNote(noteId) {
    ...
    var m = note.text.match(/^\s*(\S[^\r\n]*)[\r\n]+(\S|\S.*\S)\s*[\r\n],PUBLISH:CATEGORY:([a-zA-Z0-9_-]+(?:,[a-zA-Z0-9_-]+)*)\s*$/s)
    if(!m) {
        return
    }
    if(argv.verbose) {
    ...
    filename = filename.substr(0, titleFnPartMaxLen)
